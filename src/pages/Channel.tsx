...
//   if (!ch) return <div className="p-6 text-yt">Channel not found</div>;

//   const totalViews = (channelVideos ?? []).reduce((s, v) => s + (v.views ?? 0), 0);
...
//   // Tag chips
//   const allTags = Array.from(new Set(channelVideos.flatMap((v) => v.tags ?? [])));
//   const [chip, setChip] = useState("All");
//   const filtered = useMemo(
//     () => (chip === "All" ? uploads : uploads.filter((v) => (v.tags ?? []).includes(chip))),
//     [chip, uploads]
//   );
...
//               {/* Name + stats */}
//               <h1 className="text-2xl font-bold leading-tight text-yt">{ch.name}</h1>
