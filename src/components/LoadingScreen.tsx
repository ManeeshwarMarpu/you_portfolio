...
export default function CinematicIntro({ 
  ...
  useEffect(() => {
    const sequence = async () => {
      ...
      // 2. Shortened "Glow" duration (1.8s is plenty for branding)
      await new Promise(resolve => setTimeout(resolve, 1800));
      
      // 3. Initiate Warp Speed & Flash
      setShowSubtitle(false);
      setIsZooming(true);
      
      // We don't 'await' this one, so we can trigger onFinish mid-way
      controls.start({ 
        scale: 80, 
        opacity: 0, 
        filter: "blur(20px)",
      ...
      // 4. THE HANDSHAKE: Trigger Home reveal while screen is white (300ms in)
      setTimeout(() => {
        onFinish();
      }, 350); 
    };
        ...
        <AnimatePresence>
          {showSubtitle && (
            <motion.p
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute mt-32 text-zinc-500 text-[10px] md:text-xs tracking-[1.2em] uppercase font-bold text-center w-full"
            >
              {subtitle}
            </motion.p>
          )}
